...
/*
 *  Set key type and verify the value
 *
 *  Constraints:
 *   - Full keys pool list
 *
 ...
 */
void test_set_type_value_correctly(void)
{
	struct bt_keys *key_ref;
	int type = current_params_vector->type;

	/* 'params_it' is incremented before running the test case */
	key_ref = returned_keys_refs[params_it - 1];

	bt_keys_add_type(key_ref, type);

	zassert_true(key_ref->keys == type, "bt_keys_add_type() set incorrect key type value");
}
...
void test_main(void)
{
	...
	ztest_test_suite(bt_keys_add_type_verify_type_value,
			 LISTIFY(CONFIG_BT_MAX_PAIRED, REGISTER_SETUP_TEARDOWN, (,),
				 test_set_type_value_correctly));

	ztest_run_test_suite(bt_keys_add_type_verify_type_value);
