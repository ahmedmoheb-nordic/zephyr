			   ...
			   const void *data, uint16_t length)
{
	...
	if (conn_info.rcvd_notify_counter != received_counter) {
		TERM_FAIL("expected counter : %u , received counter : %u",
...
void notify_peer(struct bt_gatt_attr *vnd_ind_attr)
{
	...
	if (tx_notify_counter <= 5) {
		TERM_PRINT("\e[96mSending notification over connection %p size %d count %d",
	...
	if (err) {
		TERM_FAIL("Couldn't send GATT notification conn %p", conn_info.conn_ref);
		return;
	}
