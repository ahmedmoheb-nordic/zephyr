...
void notify_peer(struct bt_gatt_attr *vnd_ind_attr)
{
	...
	conn_1s_tick_ref = k_uptime_get();
	err = bt_gatt_notify(NULL, vnd_ind_attr, vnd_value, dyn_data_size);

	if ((k_uptime_get() - conn_1s_tick_ref) >= 1000) {
		uint32_t time_diff = (k_uptime_get() - conn_1s_tick_ref);

		TERM_PRINT("* ***************** *");
		TERM_PRINT("* ~%u seconds passed *", time_diff / 1000);
		TERM_PRINT("* ***************** *");
		conn_1s_tick_ref = k_uptime_get();
	}
