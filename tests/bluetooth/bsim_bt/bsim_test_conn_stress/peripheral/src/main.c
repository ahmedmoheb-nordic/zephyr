...
void notify_peer(struct bt_gatt_attr *vnd_ind_attr, uint32_t count)
{
	int err;

	memset(vnd_value, 0x00, sizeof(vnd_value));
	snprintk(vnd_value, notification_size, "%s%u", NOTIFICATION_DATA_PREFIX, count);
...
void test_peripheral_main(void)
{
	...
	while (true) {
		...
		if (simulate_vnd && vnd_ind_attr) {
			...
			notify_peer(vnd_ind_attr, tx_notify_counter++);
