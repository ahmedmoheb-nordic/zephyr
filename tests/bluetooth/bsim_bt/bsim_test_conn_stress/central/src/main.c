...
{
	if (!bt_addr_le_cmp(&conn_info_ref->peer_addr, addr)) {
		return;
	}
	...
	if (!bt_addr_le_cmp(&conn_info_ref->peer_addr, BT_ADDR_LE_ANY)) {
		...
		FAIL("Couldn't set peer address");
	}
}
...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	if (conn_err) {
		TERM_FAIL("Failed to connect to %s (%u)", addr, conn_err);
...
static void notify_peer(struct bt_conn *conn, void *data)
{
	...
	if ((k_uptime_get() - conn_info_ref->tx_notify_time_ref) < 250) {
		return;
	}
