			   ...
			   const void *data, uint16_t length)
{
	...
	if (conn_info_ref->notify_counter != received_counter) {
		TERM_FAIL("conn %u @ %p, expected counter : %u , received counter : %u",
			  (conn_info_ref - conn_infos), conn, conn_info_ref->notify_counter,
			  received_counter);
	}
...
static void notify_peer(struct bt_conn *conn, void *data)
{
	...
	if (err) {
		TERM_FAIL("Couldn't send GATT notification");
		return;
	}
