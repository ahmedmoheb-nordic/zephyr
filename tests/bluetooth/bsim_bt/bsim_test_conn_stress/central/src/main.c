			   ...
			   const void *data, uint16_t length)
{
	...
	if ((received_counter == 7) || (received_counter == 8) ||
	    (conn_info_ref->notify_counter % 100) == 0) {
		TERM_PRINT("[NOTIFICATION] addr %s conn %u @ %p data %s length %u cnt %u", addr,
			   (conn_info_ref - conn_infos), conn, data, length, received_counter);
	}

	if (conn_info_ref->notify_counter != received_counter) {
		TERM_FAIL("addr %s conn %u @ %p, expected counter : %u , received "
			  "counter : %u",
			  addr, (conn_info_ref - conn_infos), conn, conn_info_ref->notify_counter,
			  received_counter);
	}
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (conn_info_ref->discover_params.type == BT_GATT_DISCOVER_PRIMARY) {
		...
		if (err) {
			TERM_FAIL("Discover failed (err %d)", err);
		}
		...
		if (err && err != -EALREADY) {
			TERM_FAIL("Subscribe failed (err %d)", err);
		} else {
			struct conn_info *conn_info_ref;
			char addr[BT_ADDR_LE_STR_LEN];
...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	if (conn_info_ref->security_level < BT_SECURITY_L2) {
		...
		if (!err) {
			...
			TERM_FAIL("Failed to set security for conn %p with peer %s (%d).", conn,
				  addr, err);
		}
	}
#endif /* CONFIG_BT_SMP */
...
static void notify_peer(struct bt_conn *conn, void *data)
{
	...
	conn_info_ref = get_conn_info_ref(conn);
	if (conn_info_ref == NULL) {
		TERM_FAIL("Invalid reference returned for conn : %p", conn);
		return;
	}
	...
	if (err) {
		TERM_FAIL("Couldn't send GATT notification conn %p", conn);
		return;
	}
